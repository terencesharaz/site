...
class OptimizerStyles extends OptimizerBase
{
    ...
    public function getcontent()
    {
        ...
        $this->content = $this->injectCriticalBg();
    ...
    private function injectCriticalBg(){
        if (isset($_GET['no_critical_css']) && $_GET['no_critical_css'] == 1) {
            return $this->content;
        }
        global $TwoSettings;
        ...
        if ($two_serve_optimized_bg_image === 'on') {
            if(isset($this->critical) && isset($this->critical->critical_bg) && !empty($this->critical->critical_bg)){
                $critical_bg_file_dir = TWO_CACHE_DIR . "critical/" . $this->critical->critical_bg;
                if(file_exists($critical_bg_file_dir)){
                    ...
                    $critical_bg_css = OptimizerUtils::serve_different_sizes_for_critical_bg_image($critical_bg_data_arr);
        ...
        if(isset($critical_bg_css) && !empty($critical_bg_css)){
            $this->content = OptimizerUtils::inject_in_html($this->content, $critical_bg_css, array('</head>', 'before'));
            $this->cacheStructure->addToTagsToAdd($critical_bg_css, array('</head>', 'before'));
        }
        return $this->content;
    }
}